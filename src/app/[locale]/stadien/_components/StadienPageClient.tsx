...
const StadionMap = dynamic(
  () => import('@/components/stadien/StadionMapClient'),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-[500px] bg-[var(--color-grau)] rounded-xl animate-pulse flex items-center justify-center">
        <MapPin className="w-8 h-8 text-(--color-text-secondary)" />
      </div>
    ),
  }
)
...
export default function StadienPageClient({ stadien }: Props) {
  ...
  const usaCount = stadien.filter(s => s.land === 'USA').length
  const mexikoCount = stadien.filter(s => s.land === 'Mexiko').length
  const kanadaCount = stadien.filter(s => s.land === 'Kanada').length

  const filtered = filterLand ? stadien.filter(s => s.land === filterLand) : stadien
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex gap-2">
          {[
            ...
            { id: 'USA', label: `🇺🇸 ${t('usa')} (${usaCount})` },
            { id: 'Mexiko', label: `🇲🇽 ${t('mexiko')} (${mexikoCount})` },
            { id: 'Kanada', label: `🇨🇦 ${t('kanada')} (${kanadaCount})` },
          ].map(f => (
            <button
              key={f.id || 'all'}
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                filterLand === f.id
                  ...
                  : 'bg-[var(--color-grau)] text-(--color-text-secondary) border border-(--color-border) hover:border-[var(--color-aka)]'
              }`}
            >
              {f.label}
            </button>
      ...
      {view === 'map' && (
        <div className="mb-8">
          <StadionMap stadien={filtered} onSelect={setSelectedStadion} />
          {selectedStadion && (
      ...
      <div className="mt-8 bg-[var(--color-grau)] rounded-xl p-6 border border-(--color-border)">
        <h3 className="font-[var(--font-display)] text-lg text-[var(--color-aka)] mb-3">
          {t('zusammenfassung')}
        </h3>
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-2xl font-bold text-(--color-text-primary)">{usaCount}</p>
            <p className="text-(--color-text-muted) text-xs">🇺🇸 USA</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-(--color-text-primary)">{mexikoCount}</p>
            <p className="text-(--color-text-muted) text-xs">🇲🇽 Mexiko</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-(--color-text-primary)">{kanadaCount}</p>
